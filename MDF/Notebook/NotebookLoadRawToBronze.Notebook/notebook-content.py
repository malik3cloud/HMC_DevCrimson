...
# DEV Notes:
# Define ABFSS path IDs in global config instead of the Task tables
...
TaskKey = 24
TaskList = "{\"JobAuditKey\":153,\"TaskKey\":24,\"TaskType\":\"DatabaseTask\",\"SourceName\":\"Database\",\"SourceType\":\"sqlserver\",\"SourceDatabaseName\":\"SQLServer CrimsonX - Prod\",\"SourceSchemaName\":\"dbo\",\"SourceTableName\":\"Sector\",\"PrimaryKeyColumnList\":\"\",\"RawStoragePath\":\"CrimsonX\",\"RawStorageFileName\":\"Sector\",\"ArchiveOriginalFilesFlag\":true,\"ArchiveStoragePath\":null,\"ArchiveStorageFileName\":\"Sector\",\"BronzeWorkspaceName\":\"Dev - Crimson\",\"BronzeLakehouseName\":\"lh_bronze\",\"BronzeSchemaName\":\"Bronze\",\"BronzeTableName\":\"CrimsonXSector\",\"BronzeLoadMethod\":\"overwrite\",\"BronzeWorkspaceId\":null,\"BronzeLakehouseId\":null,\"WatermarkColumn\":\"ETLLoadDateTime\",\"SinkTableName\":\"CrimsonXSector\",\"SinkSchemaName\":\"Bronze\",\"SinkWatermarkColumn\":\"ETLLoadDateTime\",\"SinkLoadMethod\":\"overwrite\",\"IsWatermarkEnabledFlag\":false}"
...
# #### Define Notebook Input Parameters
...
# # Build bronze path
# bronze_lh_basepath = get_basepath(
#     BronzeWorkspaceId, 
#     BronzeLakehouseId
# )
...
# # Read raw file
# raw_id = tasklist['RawLakehouseId']
# raw_lh_basepath = get_basepath(
#     BronzeWorkspaceId, 
#     raw_id
#     # RawLakehouseId
# )
# if RawStoragePath.startswith("/"):
#     RawStoragePath = RawStoragePath.lstrip("/")

# raw_filepath = f"{raw_lh_basepath}/{RawStoragePath}/{RawStorageFileName}"



# # Exit if file has no data
# if not notebookutils.fs.head(raw_filepath):
#     result = {
#         "RowsCopied": 0,
#         "RowsInserted": 0,
#         "RowsUpdated": 0,
...
# # Get file format
# file_format = RawStorageFileName.split(".")[-1]



# # Resolve bronze metadata
# bronze_schema_name = BronzeSchemaName.lower
# Bronze_Object_Name = BronzeObjectName
...
# #### Read raw data. Accepted file format are below. Code needs to be updated if new file format is required.
# ###### 1. csv
# ###### 2. parquet - CayuseAPI/Snowflake and Banner datasets

# CELL ********************

# # Read raw file
# # THIS CODE IS ONLY BUILT FOR CAYUSE AND BANNER DATASETS!!! NEED TO UPDATE THIS FOR NEW SOURCES DEPENDING ON THE FILE FORMAT LANDED IN RAW
# if file_format == "csv":
#     df = (
...
#     df = spark.read.parquet(raw_filepath)
# else:
#     raise Exception("Unknown raw file format detected! Need manual inspection")
...
# #### Exit if spark dataframe has no rows
...
# bronze_path = f"{bronze_lh_basepath}/Files/{Bronze_Object_Name}"
# print(bronze_path)
# final_df = df
# final_df.write.format("parquet").mode("overwrite").save(bronze_path)
...
# # Since Parquet doesn't track inserts/updates/deletes, just use row count
# rows_inserted = rows_copied
# rows_updated = 0
# rows_deleted = 0
